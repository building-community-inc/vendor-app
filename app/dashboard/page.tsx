    ...
    <main className="flex px-10 py-24 gap-24 min-h-screen w-full flex-col justify-center">
      <section className=" flex flex-wrap gap-10 justify-center">
        {sanityUser.business ? (
          ...
          <BusinessCard
            credits={sanityUser.credits || 0}
            business={sanityUser.business}
            ownerName={`${sanityUser.firstName} ${sanityUser.lastName}`}
          />
        ) : (
          // </section>
          <NoBz />
        ...
        <footer className="flex flex-col gap-10 items-center">
          {sanityUser.business && (
            <ContactCard
              ...
              phone={sanityUser.business?.phone}
              address={`${sanityUser.business?.address1} ${sanityUser.business?.address2}`}
            />
          )}

          {sanityUser.business &&
            sanityUser.business.pdfs &&
            sanityUser.business.pdfs.length > 0 && (
