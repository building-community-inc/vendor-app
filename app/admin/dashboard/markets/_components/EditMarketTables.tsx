...
const EditMarketTables = ({ setChanges, bookedDatesForSelectedDay, editTables, sortedAvailableTables, vendor }: {
  editTables: boolean;
  ...
  const onDelete = (date: string, oldTableId: string) => {
    addChange({ type: "delete", date, oldTableId, vendorId: vendor.vendor._ref })
  }
      ...
      <ul className="flex flex-col gap-2">
        {editTables ?
          bookedDatesForSelectedDay.map((date, index) => {
            const deletedItem = changes?.find((change => change.type === "delete" && change.oldTableId === date.tableId && change.date === date.date));
            ...
            return (
              ...
              >
                ...
                <div className={`w-full bg-red-600 h-1.5 absolute top-1/2 -translate-y-1/2 left-0 opacity-80 ${deletedItem ? "" : "hidden"} `}></div>
                <SelectTable
...
const SelectTable = memo(({ initialValue, tables, setChanges, date, vendorId }: {
  initialValue: string;
  tables: { table: TTable }[];
  setChanges: Dispatch<SetStateAction<Change[] | undefined>>;
  date: string;
  ...
  const onChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    addChange({ type: "update", oldTableId: initialValue, newTableId: newValue, date, vendorId })
